...
$$ {\lambda}_{max}=\mathop{\arg\max}_{\lambda} P(X|{\lambda}_i) \qquad for \quad  i=1,...K $$

上式含义即为如何快速的找到一个HMM模型，使之对X得到最大的似然。例如手写英文字母的识别中，如何从26个HMM模型中快速计算哪个$$ \lambda $$计算出此序列的值最大，那么可以判断此observation对应于具体某个字母。

计算这个问题最大的困难在于效率，我们先来看怎样计算$$ P(X|\lambda) $$。X(x1,x2,...,xt)为我们最终观察到的序列，可以通过不同的状态获得相同的观察序列。

$$ P(X|\lambda)=\sum_{\Gamma}P(X,\Gamma|\lambda)=\sum_{\Gamma}P(X|\Gamma,\lambda) \times P(\Gamma|\lambda) $$

式中共有两个部分，一为$$ P(X|\Gamma,\lambda) $$，另一部分为$$ P(\Gamma|\lambda) $$，我们分别推导这两部分的公式，然后得出最终的时间复杂度。
...
于是可以得到：$$ P(X|\lambda)=\sum_{\Gamma}\prod_{t=1}^{T}P(x_t|q_t,\lambda) \times P(q_1|lambda)\times \prod_{t=2}^T P(q_t|q_{t-1},\lambda) $$。

我们可以看到由于每个T都有N中可能的状态，其时间复杂度为$$ \bigcirc(N^T) $$